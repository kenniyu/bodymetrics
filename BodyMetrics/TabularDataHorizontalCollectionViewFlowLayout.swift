//
//  TabularDataHorizontalCollectionViewFlowLayout.swift
//  BodyMetrics
//
...
class TabularDataHorizontalCollectionViewFlowLayout: UICollectionViewFlowLayout {
                ...
                if layoutAttributes.representedElementKind == UICollectionElementKindSectionHeader {
                    ...
                    if let firstCellAttributes = self.layoutAttributesForItemAtIndexPath(firstCellIndexPath),
                        let lastCellAttributes = self.layoutAttributesForItemAtIndexPath(lastCellIndexPath) {
                            let headerWidth: CGFloat = CGRectGetWidth(layoutAttributes.frame)
                            var origin = layoutAttributes.frame.origin
                            origin.x = min(max(contentOffset.x, CGRectGetMinX(firstCellAttributes.frame) - headerWidth), CGRectGetMaxX(lastCellAttributes.frame) - headerWidth)
